...
def calculate_max_defect_depth_longitudinal_with_stress(
    ...
    Args:
        ...
        defect_depth: Measured defect depth
        f_u: Tensile strength to be used in design (N/mm^2)
    ...
    relative_defect_depth_with_uncertainty = (q * (
                2 * pipe_diameter * defect_depth * p_corr_comp * theta * xi + pipe_diameter * gamma_m * p_corr_comp * pipe_thickness -
                2 * pipe_diameter * p_corr_comp * pipe_thickness * xi - 4 * defect_depth * f_u * gamma_m * pipe_thickness * theta * xi -
                2 * defect_depth * p_corr_comp * pipe_thickness * theta * xi + 4 * f_u * gamma_m * pipe_thickness ** 2 * xi -
                gamma_m * p_corr_comp * pipe_thickness ** 2 + 4 * gamma_m * sigma_l * pipe_thickness ** 2 +
                2 * p_corr_comp * pipe_thickness ** 2 * xi) /
                    (gamma_d * (
                                2 * pipe_diameter * defect_depth * p_corr_comp * theta * xi + pipe_diameter * gamma_m * p_corr_comp * q * pipe_thickness -
                                2 * pipe_diameter * p_corr_comp * pipe_thickness * xi - 4 * defect_depth * f_u * gamma_m * q * pipe_thickness * theta * xi -
                                2 * defect_depth * p_corr_comp * pipe_thickness * theta * xi + 4 * f_u * gamma_m * q * pipe_thickness ** 2 * xi -
                                gamma_m * p_corr_comp * q * pipe_thickness ** 2 + 4 * gamma_m * q * sigma_l * pipe_thickness ** 2 +
                                2 * p_corr_comp * pipe_thickness ** 2 * xi)))
    relative_defect_depth = relative_defect_depth_with_uncertainty - (epsilon_d * st_dev)
    return relative_defect_depth
